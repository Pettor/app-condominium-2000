...
{
	public class AnnualMeetingHelper
	{
		public static ErrorCodes.EditErrorCodes ValidateChangedAnnualMeeting(AnnualMeeting model)
		{
			using (var Context = new Condominium2000Context())
			{
				var result = ErrorCodes.EditErrorCodes.Success;

				var boardMember = Context.AnnualMeetings.FirstOrDefault(a => a.Id == model.Id);
				// Do any validation?
		...
		public static ErrorCodes.EditErrorCodes ValidateAnnualMeeting(AnnualMeeting model)
		{
			using (var Context = new Condominium2000Context())
			{
				var result = ErrorCodes.EditErrorCodes.Success;
				...
				var dbAmSv = Context.AnnualMeetings.FirstOrDefault(r => r.NameSv == model.NameSv);
				if (dbAmSv == null)
				{
					var dbAmEn = Context.AnnualMeetings.FirstOrDefault(r => r.NameEn == model.NameEn);
					if (dbAmEn != null)
					{
		...
		public static ErrorCodes.FileNameErrorCodes ValidateFileName(string fileName)
		{
			using (var Context = new Condominium2000Context())
			{
				...
				if (hasValidFileFormat)
				{
					...
					foreach (var am in Context.AnnualMeetings)
					{
						var words = am.FilePath.Split(delimiterCharsPath);
